...
public class AreaDetailExcelView extends AbstractSXSSFExcelView {
	
	private String sigungu;
	
	public AreaDetailExcelView(String sigungu) {
		this.sigungu = sigungu;
		
		setFileName("지역별 판매 상세(회원)_" + DateUtils.getToday(Const.DATETIME_FORMAT) + ".xlsx");
	}
	...
	private void buildItemSheet(SXSSFWorkbook workbook, List<ShopItemDetailStatistics> list) {
		...
		HeaderCell[] headerCells = new HeaderCell[]{
			...
			new HeaderCell(6000, 	"웹 아이디"),
			new HeaderCell(4000, 	"거래처 코드"),
			new HeaderCell(5000, 	"우편번호"),
			new HeaderCell(15000, 	"주소"),
			new HeaderCell(2000, 	"건수"),
			new HeaderCell(10000, 	"매출액")
		};
