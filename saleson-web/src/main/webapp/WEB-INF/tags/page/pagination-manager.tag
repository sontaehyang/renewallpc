...
<p class="pagination op-pagination">
	...
	<c:if test="${pagination.previousPage > 0}">
		<a href="${fn:replace(pagination.link, '[page]', 1)}" class="first">&lt;&lt;</a>
		<a href="${fn:replace(pagination.link, '[page]', pagination.previousPage)}" class="prev">&lt;</a>
	</c:if>
	<c:if test="${pagination.previousPage == 0}">
	...
	<c:if test="${pagination.nextPage > 0}">
		<a href="${fn:replace(pagination.link, '[page]', pagination.nextPage)}" class="next">&gt;</a>
		<a href="${fn:replace(pagination.link, '[page]', pagination.totalPages)}" class="last">&gt;&gt;</a>
	</c:if>
	<c:if test="${pagination.nextPage == 0}">
