...
<p class="pagination op-pagination">
	<c:if test="${pagination.previousPage > 0}">
		...
		<c:set var="pageLink">${fn:replace(pagination.link, '?page=[page]&', '?')}</c:set>
      	...
      	<c:set var="pageLink">${fn:replace(pageLink, '&page=[page]&', '&')}</c:set>
		<c:set var="pageLink">${fn:replace(pageLink, '&page=[page]', '')}</c:set>
      	<a href="${pageLink}" class="first"><img src="/content/images/btn/btn_first.gif" alt="First Page" /></a>
		...
		<c:if test="${pagination.previousPage == 1}">
      		...
      		<c:set var="pageLink">${fn:replace(pageLink, '&page=[page]&', '&')}</c:set>
		    <c:set var="pageLink">${fn:replace(pageLink, '&page=[page]', '')}</c:set>
      		<a href="${pageLink}" class="prev"><img src="/content/images/btn/btn_prev.gif" alt="Previous Page" /></a>
      	</c:if>
      	<c:if test="${pagination.previousPage > 1}">
