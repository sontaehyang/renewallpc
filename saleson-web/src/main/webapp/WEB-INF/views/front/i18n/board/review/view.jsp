		...
		<div class="board_view">
			...
			<div class="text">
				...
				<c:if test="${imageFileCount > 0}">
					<p class="img">
						<c:forEach items="${board.boardFiles}" var="file">
							<c:if test="${file.uploadType == '2'}">
							<img src="/upload/${boardContext.boardCode}/${board.boardId}/thumb/600x0_${file.fileId}.${file.fileType}" alt="" />
							</c:if>
						</c:forEach>
					</p>
				</c:if>
				...
				<c:if test="${boardContext.boardCfg.useSocial == '1' && !requestContext.opmanagerPage && !boardInfo.ngoPage}">
					<p class="sns">
			...
			<p class="btns">
				...
				<span class="right">
					...
					<c:if test="${board.userId == requestContext.user.userId && (board.statusCode == '1' || board.statusCode == '7')}">
