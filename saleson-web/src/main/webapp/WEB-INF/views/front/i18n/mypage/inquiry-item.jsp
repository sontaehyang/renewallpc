	...
	<div id="contents" class="pt0">
		...
		<div class="contents_inner">
			...
			<div class="mypage_search_area">
				<form:form modelAttribute="qnaParam" action="/mypage/inquiry-item" method="get">
			 		<fieldset>
						<dl>
							<dt>기간별 조회</dt>
							<dd>
			...
			<div class="btn_wrap pt10">
				<div class="btn_left">
					<button type="button" onclick="location.href='/mypage/inquiry-item'" class="btn btn-m btn-default" title="초기화"><img src="/content/images/icon/icon_reset.png" alt="초기화">초기화</button>
				</div> 
			</div><!--// btn_wrap E-->
			
			<div class="list_info clear">
				<p class="pt10">상품Q&amp;A 총 <span>${count }</span>건</p>
			</div><!--// list_info E-->
				...
				<table cellpadding="0" cellspacing="0" class="board-list mypage_qna">
		 			...
		 			<tbody>
		 				...
		 				<c:forEach items="${qnaList}" var="list" varStatus="i">
		 					...
		 					<tr id="questTit${i.count}" class="tit-off">
			 					...
			 					<td class="tleft item_name">
			 						<div class="item">
			 							<img src="${ shop:loadImage(list.itemUserCode, list.itemImage, 'XS') }" alt="${list.itemName }">
			 						</div>
			 						<div class="cont">
		 				...
		 				<c:if test="${empty qnaList}">
							<tr>
			 					<td colspan="6">
			 						<div class="coupon_not">등록된 상품Q&amp;A가 없습니다.</div>
			 					</td> 
			 				</tr> 
						</c:if>	
...
function inquiryDelete(qnaId) {
	if (confirm("해당 문의 내용을 삭제하시겠습니까?")) {
		location.href = "/mypage/inquiry-delete/inquiry-item/" + qnaId;
	}
}
