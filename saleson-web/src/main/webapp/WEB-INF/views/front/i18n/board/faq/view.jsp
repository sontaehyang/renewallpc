			...
			<p class="btns">
				...
				<span class="right">
					...
					<c:if test="${boardContext.boardAuthority.writeAuthority}">
						<!-- 
						<a href="${boardContext.boardBaseUri}/write?url=${url}" class="btn ${btnClass}">글쓰기</a>
						-->
					</c:if>
					<c:if test="${boardContext.boardAuthority.replyAuthority && boardContext.boardCfg.useReply == 1}">
						<!--
						<a href="${boardContext.boardBaseUri}/${board.boardId}/reply?url=${requestContext.currentUrl}" class="btn gray">답변</a>
						-->
					</c:if>
...
<c:if test="${board.statusCode == '1' || board.statusCode == '8'}">
...
function accept(boardId) {
	...
	Common.confirm(approvalMessage, function() {
		$.post(url("${boardContext.boardBaseUri}/" + boardId + "/accept-board-status"), {}, function(response) {
