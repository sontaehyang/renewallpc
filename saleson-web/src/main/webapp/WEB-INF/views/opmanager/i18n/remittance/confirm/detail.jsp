...
<h3 class="mt10">[${seller.sellerName}] 상품 정산 상세 - ${op:date(remittanceParam.startDate)}</h3>
...
<div class="board_write">
			...
			<tbody>
				<c:forEach items="${ list }" var="item" varStatus="index">
					<tr ${item.remittanceStatusCode == '2' ? 'style="background-color:#f3aaaa"' : ''}>
						...
						<td>${op:datetime(item.remittanceDate)}</td>
						<td>${item.itemName}</td>
						...
						<td class="text-right">${item.commissionRate}%</td>
						<td>
							<c:choose>
	...
	<div class="btn_all">
		...
		<div class="btn_right mb0">
			...
			<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/remittance/confirm/list?${queryString}" class="btn btn-active btn-sm">목록으로</a>
		</div>
	</div> 
