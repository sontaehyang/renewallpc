...
<h3><span>${op:message('MENU_13102')}</span></h3>
<form:form modelAttribute="campaign" method="post" action="/opmanager/campaign/create">
    ...
    <div id="campaignBatchDto">
        ...
        <input type="hidden" name="where" value="${campaign.id == null ? campaignBatchDto.where : campaign.searchWhere}"/>
    ...
    <table class="board_write_table">
        ...
        <tr>
            <td class="label">발송시간</td>
            <td>
                <div>
                    <p class="text-info text-sm">
                        * 발송시간 이후에는 수정이 불가능합니다.
                    </p>
                    <input type="radio" id="reserve1" name="reserve" value="0" title="즉시" checked="checked"><label
                        for="reserve1">즉시</label>
                    <input type="radio" id="reserve2" name="reserve" value="1" title="예약발송" ${campaign.sendDate != null ? 'checked="checked"' : ''}><label
                        for="reserve2">예약발송</label>
                    <span class="datepicker">
                        <form:input path="sendDate" class="datepicker" maxlength="8" title="예약발송날짜"/>
                    </span>
                    <select id="sendTime" name="sendTime">
                        <c:forEach var="i" begin="0" end="23">
                            <c:if test="${i < 10 }">
                                <option value="0${i}" label="0${i}시"}/>
                            </c:if>
                            <c:if test="${i >= 10 }">
                                <option value="${i}" label="${i}시"/>
                            </c:if>
                        </c:forEach>
                    </select>
                </div>
        ...
        <tr>
            ...
            <td>
                ...
                <div>
                    ...
                    <img id="foo" src="${campaign.imageUrl}" class="item_image size-100 "/>
                    <form:hidden path="imageUrl" value="${campaign.imageUrl}"/>
                </div>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td class="ga">
                ...
                <div id="campaignUrlArea">
                    <c:forEach var="url" items="${campaign.urlList}" varStatus="i">
                        <div>
            ...
            <td class="ga">
                <div>
                    <p>
                        ...
                        <input type="text" title="utm_medium" name="utmMedium" id="utmMedium" value="${campaign.utmMedium}" class="form-half" placeholder="매체 및 광고형태 (예시: event)"/>
                    </p>
                    <p class="pt5">
                        <span>utm_campaign</span>
                        <input type="text" title="utm_campaign" id="utmCampaign" name="utmCampaign" value="${campaign.utmCampaign}" class="form-half" placeholder="목적 (예시 : 202010-mms-event)"/>
                    </p>
                    <p class="pt5">
                        <span>utm_item</span>
                        <input type="text" title="utm_item" id="utmItem" name="utmItem" value="${campaign.utmItem}" class="form-half" placeholder="유료 키워드 설정 (예시: naver)"/>
                    </p>
                    <p class="pt5">
                        <span>utm_content</span>
                        <input type="text" title="utm_content" id="utmContent" name="utmContent" value="${campaign.utmContent}" class="form-half" placeholder="유료 광고일 경우 구분 수단"/>
                    </p>
                </div>
...
<page:javascript>
    <script type="text/javascript">
        ...
        $(function () {
            ...
            var sendDateTime = '${campaign.sendDate}';
            if (sendDateTime == '') {
                $('#sendDate').attr('disabled', 'disabled');
                $('.ui-datepicker-trigger').attr('disabled', 'disabled');
                $('#sendTime').attr('disabled', 'disabled');
            } else {
                $('#sendDate').val(sendDateTime.substring(0, 8));
                $('#sendTime').val(sendDateTime.substring(8, 10));
            }
            ...
            $("#campaign").validator(function () {

                var campaignId = $('input[name=campaignId]').val();
                if (campaignId != '') {
                    $('#campaign').attr('action', '/opmanager/campaign/create/' + campaignId);
                }

                if ($('#reserve2').is(':checked')) {
                    ...
                    var systemDate = systemNowYear + systemNowMonth + systemNowDay + hour;
                    var checkDate = $('#sendDate').val() + $('#sendTime').val();

                    if (!Common.validateDate(checkDate)) {
                        alert('예약발송 날짜를 YYYYMMDD 형식으로 정확히 입력해 주세요.');
                        $('#sendDate').val('').focus();
                        return false;
                    }

                    if (checkDate < systemDate || checkDate == systemDate) {
                        alert('예약발송일시를 현재일시 이후로 설정해주세요.');
                        $('#sendDate').focus();
                        return false;
                    }
                }
            ...
            $('input[name=reserve]').on('change', function () {
                $('#sendDate').attr('disabled', 'disabled').val('');
                $('.ui-datepicker-trigger').attr('disabled', 'disabled');
                $('#sendTime').attr('disabled', 'disabled').find('option:eq(0)').prop('selected', true);

                if ($(this).val() == 1) {
                    $('#sendDate').removeAttr('disabled');
                    $('.ui-datepicker-trigger').removeAttr('disabled');
                    $('#sendTime').removeAttr('disabled');
                    $('#sendDate').addClass('required');
                }
            });

            $('#sendTest').on('click', function () {

                if ($('#sendType2').is(':checked')) {
                    if ($('#pushToken').val() == '') {
            ...
            $('#couponList').on('click', function () {
                var sendDate = $('input[name=sendDate]').val(),
                    ...
                    url = '/opmanager/coupon/campaign-list?levelId=' + levelId + '&sendDate=';

                // 예약발송일 경우
                if ($('#reserve2').is(':checked')) {
                    if (sendDate === '') {
                        alert("예약발송날짜 항목을 입력해주세요.");
                        return false;
                    } else {
                        url = '/opmanager/coupon/campaign-list?levelId=' + levelId + '&sendDate=' + sendDate + sendTime;
                    }
                }
