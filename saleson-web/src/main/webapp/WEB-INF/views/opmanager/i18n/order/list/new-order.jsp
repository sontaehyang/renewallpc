...
<form:form modelAttribute="orderParam" action="${requestContext.requestUri}" method="get">
	...
	<div class="board_write">
		...
		<div class="btn_all">
			<div class="btn_left">
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/new-order'"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}<!-- 초기화 --></button>
			</div>
			<div class="btn_right">
			...
			<tbody>
                ...
                <c:forEach items="${list}" var="orderItem" varStatus="index">
					...
					<tr style="background:${tdColor};">
						<td>
							<input type="checkbox" name="id" value="${orderItem.itemKey}" id="${orderItem.approvalTypeLabel}" data-item-sequence="${orderItem.itemSequence}" />
						</td>
						<td>${op:numberFormat(pagination.itemNumber - index.count)}</td>
						<td>${op:datetime(orderItem.payDate)}</td>
						<td>
							<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/new-order/order-detail/${orderItem.orderSequence}/${orderItem.orderCode}">${orderItem.orderCode}</a>
						</td>
						<td>
							${orderItem.approvalTypeLabel}
							<%--
							<c:if test="${orderItem.isPoint == '1'}">
								<br>${op:message('M00246')}
							</c:if>
							--%>
							<c:if test="${orderItem.approvalTypeLabel == '은행 입금'}">
						...
						<td class="left">
							...
							<c:forEach items="${additionList}" var="addition">
								<c:if test="${orderItem.orderCode == addition.orderCode && orderItem.itemSequence == addition.parentItemSequence}">
									...
									<input type="checkbox" name="id" value="${itemKey}" id="${orderItem.approvalTypeLabel}" style="display:none"
									...
									<input type="hidden" name="shippingReadys[${additionIndex}].key" value="${itemKey}" />
									<input type="hidden" name="shippingReadys[${additionIndex}].quantity" value=${addition.shippingReadyPossibleQuantity}>
						...
						<td>
							<input type="hidden" name="shippingReadys[${itemIndex}].key" value="${orderItem.itemKey}" />
							<select name="shippingReadys[${itemIndex}].quantity">
								<c:forEach begin="1" end="${orderItem.shippingReadyPossibleQuantity}" step="1" var="quantity">
									<option value="${quantity}" ${op:selected(quantity, orderItem.shippingReadyPossibleQuantity)}>${quantity}개</option>
								</c:forEach>
							</select>
...
<script type="text/javascript">
	...
	function downloadOrderExcel(totalCount) {
		...
		location.href = "${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/new-order/order-excel-download" + param;
	}
