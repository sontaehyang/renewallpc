...
<form:form modelAttribute="item" method="post">
	...
	<div class="board_write">
		<table class="board_write_table" summary="메인 관리">
			...
			<tbody>
				...
				<tr>
					<td class="label">스팟 기간 구분 *</td>
					<td>
						<div>
							<form:radiobutton path="spotDateType" value="1" label="시점" checked="checked" />
	...
	<div class="tex_c mt20">
		...
		<a href="javascript:Link.list('/opmanager/display/spot/list')" class="btn btn-default">${op:message('M00037')}</a>	<!-- 취소 -->
	</div>
</form:form>
		...
		$.each($table.find('tr'), function() {
			...
			if ($(this).find('input[name="id"]').prop('checked') == true) {
                ...
                if (isError) {
                    ...
                    if (way == 'per') {
                        discountAmount = salePrice * discountAmount * 0.01;
	...
	$("#item").validator(function() {
		...
		if ($('input[name="spotItemIds"]').size() == 0) {
            ...
            $.each($table.find('tr'), function() {
                ...
                var salePrice = parseInt($(this).find('input[name="id"]').data('itemSalePrice'));
                var discountLimitRate = 100 - (parseInt($(this).data().commissionrate) + 30);
                var spotDiscountAmount = parseInt($(this).find('input[name="spotDiscountAmounts"]').val());
...
function findItemCallback(id, item) { 
	...
	var html = '';
    html += '<tr id="' + key + '" data-saleprice="' + item.itemSalePrice + '" data-commissionrate="' + item.commissionRate + '">';
