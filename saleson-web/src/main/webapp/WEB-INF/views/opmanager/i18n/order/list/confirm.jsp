...
<form:form modelAttribute="orderParam" action="${requestContext.requestUri}" method="get">
	...
	<div class="board_write">
		...
		<div class="btn_all">
			<div class="btn_left"> 
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/confirm'"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}<!-- 초기화 --></button>
			</div> 
			<div class="btn_right">
			...
			<tbody>
				<c:forEach items="${list}" var="orderItem" varStatus="index">
					...
					<tr>
						...
						<td>
							<a href="${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/confirm/order-detail/${orderItem.orderSequence}/${orderItem.orderCode}">${orderItem.orderCode}</a><br />
						</td>
						<td>
							${orderItem.userName}
						...
						<td>
							${op:datetime(orderItem.confirmDate)}
						</td> 
...
<script type="text/javascript"> 
    ...
    /*function downloadOrderExcel(totalCount) {
        ...
        location.href = "${requestContext.sellerPage ? '/seller' : '/opmanager'}/order/confirm/order-excel-download?" + $('#orderParam').serialize();
    }*/
