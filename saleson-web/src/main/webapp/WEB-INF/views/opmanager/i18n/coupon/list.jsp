...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
	...
	<!-- 본문 -->
	<div class="item_list">
		<h3><span>${op:message('M01137')}</span></h3> <!-- 쿠폰리스트 -->
		<form:form modelAttribute="couponParam" method="get">
			...
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/coupon/list';"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}</button> <!-- 초기화 -->
				</div>
				<div class="btn_right">
		...
		<div class="board_list mt20">
					...
					<tbody>
						<c:forEach items="${couponList}" var="list" varStatus="i">
							<tr>
								...
								<td>${list.couponTargetTimeTypeLabel}<br>
									<c:choose>
										<c:when test="${list.couponOfflineFlag == 'Y'}">(오프라인쿠폰)</c:when>
										<c:when test="${list.directInputFlag == 'Y'}">(직접입력쿠폰)</c:when>
									</c:choose>
								</td>
								<td>
										${list.couponName}<br/>
									<c:if test="${list.directInputFlag == 'Y'}">
										[${list.directInputValue}]
									</c:if>
								</td>
								<td>
								...
								<td>
									<a href="/opmanager/coupon-use/detail/${list.couponId}" class="btn btn-gradient btn-xs">사용내역</a> <!-- 복사 -->
								</td>
								<td>
									<a href="/opmanager/coupon/copy-create/${list.couponId}?url=${requestContext.currentUrl}" class="btn btn-gradient btn-xs">${op:message('M00788')}</a> <!-- 복사 -->
									<c:choose>
										<c:when test="${list.dataStatusCode == '0'}">
											<a href="/opmanager/coupon/edit/${list.couponId}?url=${requestContext.currentUrl}" class="btn btn-gradient btn-xs">수정</a>
											<a href="javascript:;" class="btn btn-active btn-xs op-coupon-publish" data-coupon-id="${list.couponId}">발행</a>
										</c:when>
										<c:otherwise>
											<c:if test="${list.couponOfflineFlag == 'Y' and list.dataStatusCode != '0'}">
												<p><a href="javascript:;" class="btn btn-success btn-xs op-offline-coupon-download" data-coupon-id="${list.couponId}">쿠폰번호 다운로드</a></p>
											</c:if>
										</c:otherwise>
									</c:choose>
								</td>
			...
			<div class="btn_all">
				...
				<div class="btn_right mb0">
					<a href="/opmanager/coupon/create?url=${requestContext.currentUrl}" class="btn btn-active btn-sm"><span class="glyphicon glyphicon-plus"></span> 쿠폰 등록</a> <!-- 쿠폰 등록 -->
				</div>
			</div>
...
<script type="text/javascript">
	$(function(){
		$('#delete_list_data').on('click', function() {
			Common.updateListData("/opmanager/coupon/list/delete", "선택된 쿠폰을 삭제하시겠습니까?");	// 선택된 데이터를 수정하시겠습니까?
		});
		...
		//발행된 오프라인 쿠폰을 엑셀 파일로 다운로드[2017-09-12]minae.yun
		$('.op-offline-coupon-download').on('click', function(){
			var couponId = $(this).data('couponId');
			if (confirm("오프라인코드목록 엑셀을 다운로드 하시겠습니까?")) {
				location.href = "/opmanager/coupon/offline-coupon-download/"+couponId;
			}
		});
	...
	function couponUpdate(couponId, mode) {
		$.post('/opmanager/coupon/'+ mode +'/' + couponId, null, function(resp) {
			if (resp.isSuccess) {
				alert('처리 되었습니다.');
				location.reload();
			} else {
