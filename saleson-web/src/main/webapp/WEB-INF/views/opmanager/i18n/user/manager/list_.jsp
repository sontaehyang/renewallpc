...
<%@ taglib prefix="op" 		uri="/WEB-INF/tlds/functions" %>
	...
	<div class="board_write">
		<form:form modelAttribute="searchParam" cssClass="opmanager-search-form clear" method="post">
			...
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/user/manager/list';"><span>${op:message('M00047')}</span></button>
				</div>
				<div class="btn_right">
			...
			<tbody>
				<c:forEach items="${list}" var="user" varStatus="i">
					<tr>
						...
						<td>
							<div>
								<a href="javascript:;" onclick="userDetail('${user.userId}');" class="table_btn" title="${op:message('M00087')} ">${op:message('M00087')}</a>
								<a href="javascript:;" onclick="userDelete('${user.userId}');" class="table_btn" title="${op:message('M00074')} ">${op:message('M00074')}</a>
							</div>
						</td>
					</tr>
				</c:forEach>
	...
	<div class="btn_all">
		<div class="btn_left">
			<button type="button" onclick="Common.deleteCheckedList(url('/opmanager/user/manager/delete/list'))" class="btn btn-default btn-sm"><span>${op:message('M00576')} <!-- 선택삭제 --></span></button>
		</div>
		<div class="btn_right">
			<a href="/opmanager/user/manager/create" class="btn btn-active btn-sm">${op:message('M00088')} </a>
		</div>
	</div>
