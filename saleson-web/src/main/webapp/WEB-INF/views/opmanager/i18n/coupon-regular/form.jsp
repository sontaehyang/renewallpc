		...
		<div class="board_write">				
			<table class="board_write_table" summary="${op:message('M01155')}">
				...
				<tbody>
					...
					<tr>
						<td class="label">정기발행 가능기간</td> 
						<td>
							<div>
								<p>
									<form:radiobutton path="couponIssueType" value="0" label="${op:message('M00497')}" title="정기발행 가능기간 설정" />
								</p>
								<p class="issue">
									<form:radiobutton path="couponIssueType" value="1" title="정기발행 가능기간 설정" label="시작, 종료일 설정" /> &nbsp;
									<span class="datepicker">
										<form:input path="couponIssueStartDate" class="two datepicker optional _number" maxlength="8" title="정기발행 가능기간 시작일" /> <!-- 달력 선택 -->
									</span>
									~
									<span class="datepicker">
										<form:input path="couponIssueEndDate" class="two datepicker optional _number" maxlength="8" title="정기발행 가능기간 종료일" /> <!-- 달력 선택 -->
									</span>
								</p>
							</div>
						</td>
					...
					<tr>
						...
						<td>
							<div>
								...
								<form:radiobutton path="couponTargetUserType" label="회원 선택" value="2" class="required" title="발급대상[회원]" />
										...
										<div class="section">
		 									...
		 									<ul>
			 									<c:forEach items="${levelGroup.value}" var="userLevel">
			 										<li>
			 											<span class="menu2">
			 												<label><input type="checkbox" name="selectCouponTargetUserLevels" title="회원 등급" value="${userLevel.levelId}" ${op:checkedArray(coupon.selectCouponTargetUserLevelArray, userLevel.levelId)} /> ${userLevel.levelName}</label>
			 											</span>
			 										</li>
			 									</c:forEach>
		 									</ul>
...
$("document").ready(function() {
	...
	//발급시점이 회원가입or상품구매후발행or첫구매인 경우 -> 쿠폰 다운로드 가능여부N 셋팅 [2017-09-15]minae.yun
...
function initCouponIssueType() {
	if ($('input[name="couponIssueType"]:checked').val() == "1") {
		...
		var issueStart = $('input[name="couponIssueStartDate"]').val().substring(0,8);
		$('input[name="couponIssueStartDate"]').val(issueStart);
	}
}
