...
<form:form modelAttribute="orderParam" action="" method="get">
	...
	<div class="board_write">
		...
		<div class="btn_all">
			<div class="btn_left"> 
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/order/exchange-request'"><span>${op:message('M00047')}<!-- 초기화 --></span></button>
			</div> 
			<div class="btn_right">
				...
				<c:forEach items="${ list }" var="orderShipping" varStatus="index">
					<c:forEach items="${orderShipping.orderItems}" var="orderItem">
						...
						<tr>
							...
							<td> 
								<a href="javascript:Common.popup('/opmanager/order/exchange-request/order-detail/${orderItem.orderCode}', 'exchangeRequestDetail', 1000, 700, 1);">${orderItem.orderCode}</a>
								<button type="button" class="btn btn-gradient btn-xs" onclick="Common.popup('/opmanager/order/exchange-request/process/${orderItem.orderItemId}', 'exchangeRequestProcess', 700, 500, 1)">교환처리</button>
