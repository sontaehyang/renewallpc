					...
					<tbody>
						...
						<tr>
							...
							<td>
								<div>
									[<c:choose>
										<c:when test="${returnApply.rtReason == 'ChangeOfMind'}">상품이상 없으나 구매의사 없음(구매자 변심)</c:when>
										<c:when test="${returnApply.rtReason == 'GoodsFault'}">상품에 결함이 있음</c:when>
										<c:when test="${returnApply.rtReason == 'NotArrived'}">상품이 도착하고 있지 않음</c:when>
										<c:when test="${returnApply.rtReason == 'NotDelivery'}">판매자가 상품을 발송하지 않음 </c:when>
										<c:when test="${returnApply.rtReason == 'WrongInformation'}">도착한 상품이 상품상세정보와 틀림</c:when>
										<c:when test="${returnApply.rtReason == 'WrongSizeColor'}">사이즈 또는 색상등을 잘못 선택함</c:when>
		 				...
		 				<tr>
		 					<th class="label">
		 						<form:select path="rtHoldReson" class="required" title="사유">
	 								<form:option value="ReturnDemandFee">반품추가비용 청구</form:option>
	 								<form:option value="ReturnProdUnreched">반품상품(구성상품) 미도착</form:option>
	 								<form:option value="OtherReason">기타사유</form:option>
	 							</form:select>
		 					</th>
		 					<td class="left">
	 							
		 						<div>
			 						<form:input path="rtHoldResonText" style="width:70%" class="required" title="사유" /><br/>
			 						<p class="addShippingAmount"> 
				 						<form:input path="rtShippingAmount" style="width:70%" class="required" title="추가 배송비" />원
									</p>
			 					</div>
...
<script type="text/javascript">
	$(function(){
		...
		$('select[name="rtHoldReson"]').on('change', function() {
			if ($(this).val() == 'ReturnDemandFee') {
				$('.addShippingAmount').show();
			} else {
				$('.addShippingAmount').hide();
			} 
		})
	});
