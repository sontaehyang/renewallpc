...
<form:form modelAttribute="orderParam" action="" method="get">
	...
	<div class="board_write">
				 	...
				 	<td> 
				 		<div>
				 			<form:select path="searchDateType">
				 				<form:option value="OI.CANCEL_REQUEST_FINISH_DATE" label="취소완료일" />
		...
		<div class="btn_all">
			<div class="btn_left"> 
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/order/cancel-finish'"><span>${op:message('M00047')}<!-- 초기화 --></span></button>
			</div> 
			<div class="btn_right">
				...
				<c:forEach items="${ list }" var="orderShipping" varStatus="index">
					<c:forEach items="${orderShipping.orderItems}" var="orderItem">
						...
						<tr>
							...
							<td>${op:datetime(orderItem.cancelRequestFinishDate)}</td> 
							<td>
								<a href="javascript:Common.popup('/opmanager/order/cancel-finish/order-detail/${orderItem.orderCode}', 'orderDetail', 1000, 700, 1);">${orderItem.orderCode}</a>
							</td>
							<td>
								<c:choose>
