...
<h3 class="mt10">[${seller.sellerName}] 추가금 정산 상세 - ${op:date(remittanceParam.startDate)} ~ ${op:date(remittanceParam.endDate)}</h3>
			...
			<table class="board_list_table" summary="정산내역 리스트">
				...
				<tbody>
					<c:forEach items="${ list }" var="item" varStatus="index">
						<c:set var="key">${item.addPaymentId}</c:set>
						<tr>
							...
							<td class="amount text-right">
								<c:if test="${item.addPaymentType == '2'}">-</c:if>${op:numberFormat(item.remittanceAmount)}원
							</td>
							<td class="amount text-right">
								<c:if test="${item.addPaymentType == '2'}">-</c:if>${op:numberFormat(item.remittanceAmount)}원
							</td>
							<td>
								<input type="text" name="editAddPaymentRemittanceMap[${key}].remittanceExpectedDate" class="required" value="${item.remittanceExpectedDate}" />
							</td>
						</tr>
					</c:forEach>
				</tbody>
...
<script type="text/javascript"> 
	...
	function validate() {
		...
		$.each($form.find('input[name=id]:checked'), function(){
			...
			$date = $form.find('input[name="editAddPaymentRemittanceMap['+key+'].remittanceExpectedDate"]');
			if ($date.size() == 0) {
				isError = true;
				return false;
			} else {
