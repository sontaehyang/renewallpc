...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
			...
			<div class="item_list">			
				...
				<form:form modelAttribute="itemParam" method="post">
					...
					<div class="btn_all">
						<div class="btn_left">
							<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/opmanager/main-display/spot/list';"><span>${op:message('M00047')}</span></button> <!-- 초기화 -->
						</div> 
						<div class="btn_right">
					...
					<form id="listForm">
						<table class="board_list_table" id="board_list_table" summary="전체상품리스트">
							...
							<c:forEach items="${list}" var="item" varStatus="i">
								...
								<tr id="${spotAreaKey}">
									...
									<td>
										<div>
											<a href="javascript:Link.view('/opmanager/item/edit/${item.itemUserCode}')"><img src="${item.imageSrc}" class="item_image" alt="상품이미지" /></a>
										</div>
									</td>
									...
									<td class="text-right">
										<c:choose>
											<c:when test="${item.spotType == '1'}">
												...
												&nbsp;원&nbsp;
			 									<button type="button" class="btn btn-dark-gray btn-sm" onclick="javascript:oneDiscount('${spotAreaKey}');"><span>${op:message('M01154')}</span></button> <!-- 적용 -->	
											</c:when>
											<c:otherwise>
									...
									<td>
										<c:if test="${item.spotType == '1'}">
											<a href="javascript:Link.view('/opmanager/main-display/spot/edit/${item.itemId}')" class="table_btn">수정</a>
										</c:if>
										<a href="#" class="delete_item table_btn" style="margin-top:2px;">삭제</a>
									</td>
								</tr>
							</c:forEach>
						...
						<div class="btn_all">
							<div class="btn_left mb0" style="font-size:15px; font-weight:bold;">
								...
								을 할인하여 일괄적으로 수정&nbsp;&nbsp;
								<button type="button" id="all_discount" class="btn btn-dark-gray btn-sm"><span>일괄 할인 재 수정</span></button> 
								<button type="button" id="delete_list_data" class="btn btn-dark-gray btn-sm">선택삭제</button>
							</div>
							<div class="btn_right mb0">
								<a href="/opmanager/main-display/spot/create" class="btn btn-active btn-sm"><span>스팟상품등록</span></a>
							</div>
						</div>
					</form>
...
function oneDiscount(key) {
	...
	$.post("/opmanager/main-display/spot/update/one/discount", param ,function(response){
...
$(function() {
	...
	$('#all_discount').on('click',function() {
		...
		$.post("/opmanager/main-display/spot/update/discount", $("#listForm").serialize() ,function(response){
			Common.responseHandler(response, function(){
				location.reload();
			});
		});
	...
	$('.delete_item').on('click', function(e) {
		...
		Common.updateListData("/opmanager/main-display/spot/list/delete", Message.get("M00306"));	// 선택된 데이터를 삭제하시겠습니까?
	});
