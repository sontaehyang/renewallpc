...
<div class="statistics_web">
	...
	<div class="board_list">
		...
		<table class="board_list_table stats ${statisticsParam.displaySubtotal == 'N' ? 'odd-even' : ''}">
			<caption>입점업체별 판매율</caption>
			<thead>
				<tr>
					<!-- th rowspan="2">${op:message('M00200')}</th> --> <!-- 순번 -->
					<th rowspan="2" >판매자명</th> <!-- 판매자명 -->
			...
			<tbody>
				<c:forEach items="${sellerStatsList}" var="list" varStatus="i">
					<tr class="${i.count % 2 == 0 ? 'even' : '' }">
						...
						<td rowspan="${fn:length(list.groupStats) }" style="text-align: left;">
							<a href="javascript:Common.popup('/opmanager/shop-statistics/sales/day/detail?sd=${statisticsParam.startDate}&ed=${statisticsParam.endDate}&sellerId=${list.sellerId}&where=${statisticsParam.where}&query=${statisticsParam.query}&type=detail', 'order-popup-seller', '1600', '800', 1);">
							    ${list.sellerName}
							</a>
						</td>
		...
		<!-- // 판매율별 -->
		<c:if test="${empty sellerStatsList}">
		...
		<sec:authorize access="hasRole('ROLE_EXCEL')">
			<div class="btn_all">
				<div class="right">
					<a href="/opmanager/shop-statistics/sales/seller/excel-download?${queryString}"  class="btn btn-success btn-sm"><span class="glyphicon glyphicon-save"></span>${op:message('M00254')}</a> <!-- 엑셀 다운로드 -->
				</div>
			</div>
		</sec:authorize>
