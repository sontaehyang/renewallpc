...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<form:form modelAttribute="itemParam" method="get" enctype="multipart/form-data">
			...
			<div class="board_write">
				<table class="board_write_table" summary="${op:message('M00459')}"> <!-- 이용후기(리뷰) --> 
					...
					<tbody>
						 ...
						 <tr>
						 	<td class="label">${op:message('M01429')}</td>  <!-- 추천유무 --> 
						 	<td>
						 		<div>
						 			<form:radiobutton path="recommendFlag" value="" label="전체" />
						 			<form:radiobutton path="recommendFlag" value="Y" label="${op:message('M00759')}" /> <!-- 추천 --> 
									<form:radiobutton path="recommendFlag" value="N" label="${op:message('M01430')}" /> <!-- 비추천 -->
								</div>
						 	</td>	
						 </tr>
						 <tr>
			...
			<div class="btn_all">
				<div class="btn_left">
					...
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/item/review/list';"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}</button> <!-- 초기화 -->
				</div>
				<div class="btn_right">
			...
			<div class="count_title mt20">
				<h5>
					${op:message('M00039')} : ${sellerReviewCount} ${op:message('M00743')}
				</h5>	 <!-- 전체 -->   <!-- 건 조회 --> 
				<span>
					${op:message('M00052')} : 
		...
		<form id="listForm">
					...
					<tbody>
						<c:forEach items="${sellerReviewList}" var="list" varStatus="i">
							<tr>
								...
								<td>
									<div class="item_img2">
										<img src="${list.item.imageSrc}" width="50px" heigt="50px" alt="${op:message('M00659')}"> <!-- 상품이미지 -->
									</div>
								</td>
								<td class="tex_l">
									<div>
								...
								<td>${list.userName}(${list.loginId})</td>
								<td>
									${list.starScore}
								</td>
								...
								<td>
									<a href="/seller/item/review/edit/${list.itemReviewId}" class="btn btn-gradient btn-xs">${op:message('M00087')}</a>
			...
			<!-- <div class="btn_all">
				<div class="btn_left">
					<span>선택한 이용후기(리뷰)를</span>  	
					<a href="#" id="recommend_popup" class="ctrl_btn">추천</a> 
				</div>
			</div> -->	
...
$(function() {
	...
	$('#recommend_popup').on('click', function() {
		Common.popup(url('/seller/item/review/recommend'), 'recommend', 450, 460);
		return false;
	});
	
	//목록데이터 - 삭제처리
	$('#recommend_popup').on('click', function() {
		Common.updateListData("/seller/item/review/recommend", "선택된 리뷰를 추천하시겠습니까?");	// 선택된 데이터를 삭제하시겠습니까?
	});
...
function deleteCheck(reviewId) {	
	...
	if(confirm(Message.get("M00196"))) {	// 삭제하시겠습니까? 
		$.post("/seller/item/review/delete/" + reviewId, function() {
			alert('삭제되었습니다.');
			location.reload();
		}); 
	} else {
...
function reviewDetail(itemReviewId) {
	location.href = "/seller/item/review/edit/" + itemReviewId;
}
</script>
</page:javascript>