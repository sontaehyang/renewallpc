...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
						...
						<tr>
							<div>
								<td class="label">회원여부</td>
							 	<td>
							 		<div>
							 			<form:radiobutton path="userId" value="0" checked="checked" label="${op:message('M00039')}" />
										<form:radiobutton path="userId" value="1" label="${op:message('M00465')}" /> 
										<form:radiobutton path="userId" value="2" label="${op:message('M00466')}" />
							 		</div>
							 	</td>
							 </div>	
						 </tr>
						 	...
						 	<td>
						 		<div>
									<form:select path="where" title="${op:message('M00468')}"> <!-- 키워드선택 --> 
										<form:option value="USER_ID" label="${op:message('M00081')}" /> <!-- 아이디 --> 
		...
		<!-- 버튼시작 -->	
			<div class="btn_all">
				<div class="btn_left">
					<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/qna/list'"><span>${op:message('M00047')}</span></button> <!-- 초기화 --> 
				</div>
				<div class="btn_right">
		...
		<!-- 2015.1.6 -->
		<form action="/seller/qna/list" method="post" id="listForm">
						...
						<tbody>
							<c:forEach items="${qnaList}" var="list" varStatus="i">
								<tr>
									...
									<td class="tex_l">
										<a href="/seller/qna/view/${list.qnaId}">${list.subject}</a> <!-- 제목 -->
									</td>
									...
									<td colspan="3">
										<c:choose>
											<c:when test="${list.userId > 0}">
												<a href="javascript:Common.popup('/seller/user/popup/details/${list.userId}', '/user/popup/details', 1100, 800 ,1, 0, 0)">${list.userName}(${list.email})</a>
											</c:when>
			...
			<div class="btn_all">
				...
				<div class="btn_left">
					<a id="delete_list_data" href="#" class="ctrl_btn">
						<span>삭제</span>
					</a>
				</div> 
			</div>
...
$(function() {
	...
	$('#delete_list_data').on('click', function() {
		Common.updateListData("/seller/qna/delete", "삭제된 게시글을 복구할 수 없습니다. 정말로 삭제하시겠습니까?");// 선택된 데이터를 삭제하시겠습니까?
	});
