...
<form:form modelAttribute="orderParam" action="" method="post">
	...
	<div class="board_write">
				 	...
				 	<td> 
				 		<div>
				 			<form:select path="searchDateType">
				 				<form:option value="OI.CANCEL_REQUEST_DATE" label="취소요청일" />
		...
		<div class="btn_all">
			<div class="btn_left"> 
				<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/order/cancel-request'"><span>${op:message('M00047')}<!-- 초기화 --></span></button>
			</div> 
			<div class="btn_right">
...
<div class="board_list">
	...
	<form id="listForm">
		<table class="board_list_table" summary="주문내역 리스트">
			...
			<tbody>
				<c:forEach items="${ list }" var="orderCancelApply" varStatus="index">
					...
					<tr style="background:${tdColor};">
						<!--<td>
							<c:if test="${orderItem.orderStatus != '95'}">
								<input type="checkbox" name="id" value="${ orderCancelApply.orderCancelApplyId }" />
							</c:if>
						</td>-->
						<td>${op:numberFormat(pagination.itemNumber - index.count)}</td>
						<td>
							${op:datetime(orderItem.cancelRequestDate)}
						...
						<td>
							<a href="javascript:Common.popup('/seller/order/cancel-request/order-detail/${orderItem.orderCode}', 'cancelRequestDetail', 1000, 700, 1);">${orderItem.orderCode}</a>
							<c:if test="${orderItem.orderStatus != '95'}">
								<button type="button" class="btn btn-gradient btn-xs" onclick="Common.popup('/seller/order/cancel-request/process/${orderCancelApply.orderCancelApplyId}', 'cancelRequestProcess', 700, 500, 1)">취소처리</button>
							</c:if>
						</td> 
						<td class="left">
						...
						<td>
							<strong>${orderCancelApply.applyQuantity}개</strong>
...
<div class="board_guide ml10">
	<p class="tip">Tip</p>
	<p class="tip"><strong>"주문취소 거절"</strong>하시면 해당 주문은 <strong>"배송준비중 목록"</strong>으로 이동됩니다.</p>
	<p class="tip"><strong>"주문취소 거절"</strong>하시는 경우 해당 요청건이 배송준비중 이였으면 배송 요청일은 유지되며, 신규주문에서 넘어온경우 <strong>"주문취소 거절"</strong>처리한 날짜가 배송 요청일이 됩니다.</p>
</div>
...
<script type="text/javascript"> 
	$(function(){
		...
		$('.cancel-confirm').on('click', function() { 
			...
			if (confirm("선택하신 주문을 취소처리 하시겠습니까?")) {
				Manager.Order.listUpdate('cancel-request', 'cancel-confirm');
			}
		});
