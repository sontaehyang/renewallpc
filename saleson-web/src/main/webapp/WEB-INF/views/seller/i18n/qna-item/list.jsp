...
<%@ taglib prefix="shop"	uri="/WEB-INF/tlds/shop" %>
		...
		<form:form modelAttribute="qna" method="get" enctype="multipart/form-data">
			...
			<!-- 버튼시작 -->	
				<div class="btn_all">
					<div class="btn_left">
						<button type="button" class="btn btn-dark-gray btn-sm" onclick="location.href='/seller/qna-item/list'"><span class="glyphicon glyphicon-repeat"></span> ${op:message('M00047')}</button> <!-- 초기화 -->
					</div>
					<div class="btn_right">
					...
					<tbody>
						<c:forEach items="${itemQnaLists}" var="item" varStatus="i">
							<tr>
								...
								<td>
									<div class="item_img2">
										<img src="${ shop:loadImage(item.itemUserCode, item.itemImage, 'XS') }" width="50px" heigt="50px" alt="${op:message('M00659')}"> <!-- 상품이미지 --> 		
									</div>
								</td> 
								<td>${item.itemUserCode}</td>
								<td class="tex_l">
									<%-- <a href="/seller/qna-item/view/${item.qnaId}">${item.subject}</a> <!-- 제목 --> --%>
									<a href="javascript:qnaItemView(${item.qnaId})">${item.subject}</a> <!-- 제목 -->
								</td> 
								<td>
									<c:choose>
										<c:when test="${item.userId > 0}"> <!-- 이름/이메일 -->
											<%-- <a href="javascript:Common.popup('/seller/user/popup/details/${item.userId}', '/user/popup/details', 1100, 800 ,1, 0, 0)"> --%>
...
$(function() {
	...
	$('#delete_list_data').on('click', function() {
		Common.updateListData("/seller/qna-item/delete", "삭제된 게시글을 복구할 수 없습니다. 정말로 삭제하시겠습니까?");// 선택된 데이터를 삭제하시겠습니까?
...
function qnaItemView(qnaId) {
	$('#qna').attr("action", "/seller/qna-item/view/"+qnaId).submit();
}
</script>
