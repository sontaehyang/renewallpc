package saleson.shop.seller.shipment;
...
import saleson.shop.shipment.ShipmentManagerController;
...
private static final Logger log = LoggerFactory.getLogger(ShipmentManagerController.class);
	...
	public String listPopup(@PathVariable("sellerId") long sellerId, Model model, Shipment shipment) {
		...
		return "view:/seller/shipment/list-popup";
	}
	
	@PostMapping("create")
	public String create(HttpServletRequest request, Shipment shipment) {
		shipment.setSellerId(SellerUtils.getSellerId());
		...
		if (request.getHeader("referer").indexOf("popup") > -1) {
			...
			return "redirect:/seller/shipment/list-popup/" + shipment.getSellerId();
			
		}
		
		return "redirect:/seller/shipment/list";
	}
