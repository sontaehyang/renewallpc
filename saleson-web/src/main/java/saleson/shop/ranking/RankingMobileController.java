...
@RequestProperty(title="랭킹관리", layout="default", template="mobile")
public class RankingMobileController {
	...
	public String code(@PathVariable("code") String code, Model model) {
		...
		rankingParam.setCategoryTeamCode(code);	// 1차 카테고리 코드
		rankingParam.setLimit(10);
		
		List<Item> rankingList = rankingService.getRankingListForFront(rankingParam);
		
		model.addAttribute("items",rankingList);
		model.addAttribute(code,"class=\"on\"");
	...
	public String rankingCode(@ModelAttribute("rankingParam") RankingParam rankingParam , @PathVariable("rankingCode") String rankingCode, Model model) {
		String code = "";
		...
		model.addAttribute("groupCategoryList",categoriesService.getCategoryGroupRanking(code));
		model.addAttribute(code,"class=\"default\"");
		model.addAttribute("code",code);
